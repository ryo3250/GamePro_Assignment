...
[[nodiscard]] bool depth_buffer::create() noexcept {
...
[[nodiscard]] ID3D12Resource* depth_buffer::depthBuffer() const noexcept {
    assert(depthBuffer_ && "デプスバッファが未生成です");
    return depthBuffer_.Get();
}

[[nodiscard]] D3D12_CPU_DESCRIPTOR_HANDLE depth_buffer::getCpuDescriptorHandle() const noexcept {
    assert(depthBuffer_ && "デプスバッファが未生成です");
    return handle_;
}